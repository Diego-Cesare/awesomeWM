...
awesome.connect_signal("theme::colors", function(colors)
    ...
    next_image:get_children_by_id("next")[1]:set_markup(
        maker.text(colors.fg, "Regular 10", ""))
    prev_image:get_children_by_id("prev")[1]:set_markup(
        maker.text(colors.fg, "Regular 10", ""))
end)
...
local wall =
    maker.image(icons.wallpaper, colors.transparent, 3, 0, "image_icon")
