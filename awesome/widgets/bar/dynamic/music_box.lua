...
awesome.connect_signal("play::metadata", function(title, artist)
  ...
  if artist == "Artist" then
    song_description:set_markup(maker.text(colors.fg, "Bold 10", "Parado agora"))
  else
    song_description:set_markup(maker.text(colors.orange, "Bold 10", "Tocando agora"))
  end
end)

player_img:buttons(gears.table.join(
